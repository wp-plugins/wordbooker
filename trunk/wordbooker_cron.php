...
function wordbook_poll_facebook() {
	...
	if (DEBUG) {$debug_string=date("Y-m-d H:i:s",time())." : Cron Running\n";
	fwrite($fp, $debug_string);}
	if ( !isset($wordbook_settings['wordbook_comment_get'])) {
		if (DEBUG) {$debug_string=date("Y-m-d H:i:s",time())." : Comment Scrape not active. Cron Finished\n";
		fwrite($fp, $debug_string);}
 		fclose($fp); return;}
	$debug_file='/tmp/wordbook_'.$table_prefix.'debug';
	...
	foreach ($wb_users as $wb_user){
		if (DEBUG) {$debug_string="Processing data for user ".$wb_user->user_id."\n";
		fwrite($fp, $debug_string);}	
	...
	if (DEBUG) {$debug_string=date("Y-m-d H:i:s",time())." : Checking to see if we have any posts to check for comments\n";
	fwrite($fp, $debug_string);}
	...
	$sql='Select fb_post_id,comment_timestamp,wp_post_id from ' . WORDBOOK_POSTCOMMENTS . ' where fb_post_id like "'.$fbclient->users_getLoggedInUser().'%" order by fb_post_id desc ';
		if (DEBUG) {$debug_string= $sql."\n";
		fwrite($fp, $debug_string);}	
	$rows = $wpdb->get_results($sql);
	// For each FB post ID we find we go out to the stream on Facebook and grab the comments.
	if (count($rows)>0) {foreach ($rows as $comdata_row) {
		#$fbcomments=$fbclient->stream_getComments($comdata_row->fb_post_id);
...
$fbcomments=$fbclient->fql_query($fbsql);
		...
		if (DEBUG) {$debug_string="Number of comments to process for post ".$comdata_row->fb_post_id." is ".count($fbcomments) ."\n";
		fwrite($fp, $debug_string);}	
		foreach ($fbcomments as $comment) {
			if (DEBUG) {$debug_string="incoming comment time is ".$comment[time]." and the last recorded comment time stamp was ".$comdata_row->comment_timestamp."\n";
			fwrite($fp, $debug_string);}
			...
			if ($comment[time]>$comdata_row->comment_timestamp) {
				...
				#sleep(5);
				if (DEBUG) {$debug_string="Comment : ".$comment[text]." was made at ".date("Y-m-d H:i:s",$comment[time])." by ".$fbuserinfo[0][name]." (".$comment[fromid].")\n";
				fwrite($fp, $debug_string);}
		...
		if (DEBUG) {$debug_string="No new comments to process\n";
		fwrite($fp, $debug_string);}	}
		}
	}
		if (DEBUG) {$debug_string=date("Y-m-d H:i:s",time())." : No comments to process for post :".$comdata_row->fb_post_id."\n";
		fwrite($fp, $debug_string);}}}}
		if (DEBUG) {
