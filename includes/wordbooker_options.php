...
function worbooker_validate_options($options) {
	...
	if (isset( $_POST["RSD"] ))  {
		...
		$options["wordbooker_attribute"]= __("Posted a new post on their blog");
...
function wordbooker_option_manager() {
	...
	if(isset($_POST['user_meta'])) {
		...
		#	$wordbookeruser_settings['wordbooker_use_excerpt']=$_POST['wordbooker_use_excerpt'];
			...
			if (isset($_POST['rwbus'])) {delete_usermeta( $user_ID, $wordbooker_user_settings_id );$ol_flash =  __("Your user level settings have been reset.");} else {
...
function wordbooker_blog_posting_options() {
		...
		echo '<label for="wb_publish_page_default">'.__("Default Publish Page to Facebook"). ' : </label>';
		...
		echo '<label for="wb_status_update">'.__("Facebook Status Attribute"). ' :</label>';
		...
		echo '<br /><label for="wb_action_link">'.__("Action Link Option "). ': </label><select id="wordbooker_actionlink" name="wordbooker_settings[wordbooker_actionlink]"  >';	
		...
		echo '<label for="wordbooker_search_this_header">'.__("Enable Extended description for Share Link"). ' :</label> ';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_search_this_header]" '.$checked_flag[$wordbooker_settings["wordbooker_search_this_header"]].' /><br />';
			echo '<label for="wb_description_meta_length">'.__('Length of Description Meta-Tag').' :</label> <select id="wordbooker_description_meta_length" name="wordbooker_settings[wordbooker_description_meta_length]" >';
...
function wordbooker_blog_facebook_options() {
		...
		$fblike_location=array('top'=>'Above Post ','bottom'=>'Below Post','coded'=>'Defined by theme template','tagged'=>'Defined by Tag in post');
		$checked_flag=array('on'=>'checked','off'=>'');
		$fblike_send=array('false'=>'No','true'=>'Yes');
		$fblike_send_combi=array('false'=>'No - use Send instead of Like','true'=>'Yes - use both Like and Send');
		...
		echo '<label for="wb_fblike_location">&nbsp;'.__('Facebook Like - Display Button ').' :</label> <select id="wordbooker_fblike_location" name="wordbooker_settings[wordbooker_fblike_location]"  >';
		...
		echo '<label for="wb_fblike_action">&nbsp;'.__('Facebook Like - Verb to Display').' :</label> <select id="wordbooker_fblike_action" name="wordbooker_settings[wordbooker_fblike_action]"  >';
		...
		echo '<label for="wb_fblike_colorscheme">&nbsp;'.__('Facebook Like - Colour Scheme').' :</label> <select id="wordbooker_fblike_colorscheme" name="wordbooker_settings[wordbooker_fblike_colorscheme]"  >';
		...
		echo '<label for="wb_fblike_font">&nbsp;'.__('Facebook Like - Display Font').' :</label> <select id="wordbooker_fblike_font" name="wordbooker_settings[wordbooker_fblike_font]"  >';
		...
		echo '<label for="wb_fblike_button">&nbsp;'.__('Facebook Like - Layout Style').' :</label> <select id="wordbooker_fblike_button" name="wordbooker_settings[wordbooker_fblike_button]"  >';
		...
		echo '<label for="wb_fblike_faces">&nbsp;'.__('Facebook Like - Display Faces (Standard layout only)').' :</label> <select id="wordbooker_fblike_faces" name="wordbooker_settings[wordbooker_fblike_faces]"  >';
		...
		echo '<label for="wb_fbshare_location">&nbsp;'.__('Facebook Share - Display Button ').' :</label> <select id="wordbooker_fbshare_location" name="wordbooker_settings[wordbooker_fbshare_location]"  >';
		foreach ($fblike_location as $i => $value) {
		...
		echo '<label for="wb_facebook_share_page">&nbsp;'.__("Show Facebook Share button on Pages"). ' : </label>';
...
function wordbooker_blog_comment_options() {
		...
		echo '<label for="wb_comment_handling">'.__("Comment handling"). ' : </label> <select id="wordbooker_comment_handling" name="wordbooker_settings[wordbooker_comment_handling]" > ';

		$arr = array(1=> __("Use Wordpress Comments and Wordbooker comment handling"), 2=> __("Use Facebook Comments"));
                foreach ($arr as $i => $value) {
		...
		echo '<label for="wb_use_fb_comments">'.__("Enable Facebook Comment handling  "). ' : </label> ';
		...
		$fb_comment_location=array('bottom'=>'Created in line below post','coded'=>'Defined by theme template','tagged'=>'Defined by Tag in post');

		echo '<label for="wb_comment_location">'.__('Comment placing ').' :</label> <select id="wordbooker_comment_location" name="wordbooker_settings[wordbooker_comment_location]"  >';
		...
		echo '<label for="wb_comment_colorscheme">'.__('Comment Box - Colour Scheme').' :</label> <select id="wb_comment_colorscheme" name="wordbooker_settings[wb_comment_colorscheme]"  >';
		...
		echo '<label for="wb_use_fb_comments_admin">'.__("All Wordbooker users can moderate comments  "). ' : </label> ';
 ...
 <br />';
		...
		echo '<label for="fb_comment_app_id">'.__("Use this Facebook Application ID for comment moderation"). ' :</label>';
		...
		echo '<label for="fb_comment_app_id">'.__("Width of comment box"). ' :</label>';
		...
		echo '<label for="fb_comment_count">'.__("Number of comments to display"). ' :</label>';
		...
		echo '<label for="wordbooker_comment_notify">'.__('Enable notification of new comments').' : </label>';
		...
		echo '<label for="wordbooker_use_facebook_comments">'.__('Facebook comment handling should be enabled on new posts').' : </label>';
		...
		echo '<label for="wb_import_comment">'.__("Import Comments from Facebook for new Wordbooker Posts"). ': </label>';
		...
		echo '<label for="wb_publish_comment_push">'.__("Push Comments up to Facebook for new posts"). ' : </label>';
		...
		echo '<label for="wb_publish_comment_handling">'.__("Enable Comment processing"). ' : </label>';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_comment_handling]" '.$checked_flag[$wordbooker_settings["wordbooker_comment_handling"]].' /> ( '.__("Next Scheduled fetch is in").' : '.date('i',(wp_next_scheduled('wb_cron_job') - time ())).' minute(s) )<br />';
...
function wordbooker_blog_advanced_options() {
		...
		echo '<label for="wb_facebook_iframe">'.__("Use Iframes instead of FBXML to render FB features"). ' : </label>';
		...
		echo '<label for="wb_fake_publish">'.__("Only Pretend to Publish on Facebook - TEST MODE"). ' : </label>';
		...
		echo '<label for="wb_disable">'.__("Disable ALL Wordbooker functionality"). ' : </label>';
...
global $ol_flash, $wordbooker_settings, $_POST, $wp_rewrite,$user_ID,$wpdb, $table_prefix,$current_blog,$blog_id,$db_prefix,$wordbooker_user_settings_id,$user_ID,$wordbooker_hook;
		...
		echo '<div class="wrap">';
		echo '<h3>'.__('User Level Settings').'</h3>';
		...
		$sql="select wpu.ID,wpu.display_name,facebook_id from $wpdb->users wpu,".WORDBOOKER_USERDATA." wud where wpu.ID=wud.user_id;";
		$wb_users = $wpdb->get_results($sql);
		$fanpages2=$fanpages;
...
$arr = array(1=> __("As a Wall Post"),  2=> __("As a Note"), 3=> __("As a Status Update" )  );
	...
	echo "</select>";
		...
		echo '<label for="wb_extract_length">'.__('Length of Extract').' : </label><select id="wordbooker_extract_length" name="wordbooker_extract_length"  >';
	        $arr = array(10=> "10",20=> "20",50=> "50",100=> "100",120=> "120",150=> "150",175=> "175",200=> "200",  250=> "250", 256=>__("256 (Default) "), 270=>"270", 300=>"300", 350 => "350",400 => "400",500 => "500",600 => "600",700 => "700",800 => "800",900 => "900");
                foreach ($arr as $i => $value) {
	...
	echo '<label for="wb_status_update">'.__('Facebook Status Text').'  : </label> ';
		...
		echo '<label for="wb_action_link">'.__('Action Link Option').' : </label><select id="wordbooker_actionlink" name="wordbooker_actionlink"  >';	
		...
		echo '<label for="wb_status_id">'.__('Show Status for').' : </label> <select name="wordbooker_status_id" ><option selected="yes" value=-100>'.__('My Own Profile').'&nbsp;&nbsp;</option>';
		$option="";
		if ($have_fan_pages==1) {	
	...
	if (defined('WORDBOOKER_PREMIUM')) { _e("You're using the Premium options in Wordbooker. You really should contribute something to the support and development of this plugin.  Please provide your FB Id number and your website when making payment so your details can be added to the 'thanks' list");
} else {
