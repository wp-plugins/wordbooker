...
function worbooker_validate_options($options) {
	...
	if (isset( $_POST["RSD"] ))  {
		...
		$options['wordbooker_allow_publish_select']=null;
		$options["wordbooker_attribute"]= __("Posted a new post on their blog", 'wordbooker');
		$options['wordbooker_clear_diagnostics']=null;
		$options["wordbooker_close_comment"]=0;
		$options["wordbooker_comment_approve"]=null;
		$options['wordbooker_comment_attribute']=__("[Comment imported from blog] - You Should Edit This",'wordbooker');
		$options['wordbooker_comment_cron']=null;
		$options['wordbooker_comment_email']=get_bloginfo( 'admin_email' );
		$options["wordbooker_comment_get"]=null;
		$options['wordbooker_comment_handling']=null;
		$options['wordbooker_comment_location']=null;
		$options['wordbooker_comment_log']=null;
		$options["wordbooker_comment_poll"]=null;
		$options['wordbooker_comment_post_format']="%tag%";
		$options["wordbooker_comment_pull"]=null;
		$options["wordbooker_comment_push"]=null;
		$options["wordbooker_comment_put"]=null;
		$options["wordbooker_default_author"]=1;
		$options['wordbooker_delete_comment_fb_exp']=null;
		$options['wordbooker_delete_comment_fb_imp']=null;
		$options['wordbooker_delete_published']=null;
		$options['wordbooker_description_meta_length']=350;
		$options['wordbooker_diagnostic_admin']=null;
		$options['wordbooker_disabled']=null;
		$options['wordbooker_disable_shorties']='off';
		$options["wordbooker_exclude_ctype"]="";
		$options['wordbooker_exclude_ptype']="";
		$options["wordbooker_exclude_ttype"]="";
		$options['wordbooker_extract_length']=256;
		$options['wordbooker_fake_publish']=null;
		$options['wordbooker_fb_comments_admin']=null;
		$options['wordbooker_fb_disable_api']=null;
		$options['wordbooker_fb_disable_og']=null;
		$options['wordbooker_fblike_action']='like';
		$options['wordbooker_fblike_align']='left';
		$options['wordbooker_fblike_button']='standard';
		$options['wordbooker_fblike_colorscheme']='dark';
		$options['wordbooker_fblike_faces']=false;
		$options['wordbooker_fblike_font']='arial';
		$options['wordbooker_fblike_location']='top';
		$options['wordbooker_fblike_send']=false;
		$options['wordbooker_fblike_send_combi']=false;
		$options['wordbooker_fbread_location']='bottom';
		$options['wordbooker_fb_rec_act']=false;
		$options['wordbooker_fbshare_location']='top';
		$options['wordbooker_iframe']=false;
		$options['wordbooker_include_ctype']="";
		$options["wordbooker_include_ptype"]="";
		$options['wordbooker_include_ttype']="";
		$options['wordbooker_like_button_category']=false;
		$options['wordbooker_like_button_frontpage']=false;
		$options["wordbooker_like_button"]=null;
		$options['wordbooker_like_button_page']=false;
		$options['wordbooker_like_button_post']=false;
		$options['wordbooker_like_button_show']=false;
		$options['wordbooker_like_share_too']=false;
		$options['wordbooker_like_width']=250;
		$options["wordbooker_meta_tag_scan"]="image,thumb,Thumbnail";
		$options['wordbooker_meta_tag_thumb']=false;
		$options['wordbooker_no_facebook_gravatars']=false;
		$options['wordbooker_no_like_stick']=true;
		$options['wordbooker_no_share_stick']=true;
		$options['wordbooker_orandpage']=2;
		$options['wordbooker_override_author']=0;
		...
		$options['wordbooker_publish_page_default']=false;
		$options['wordbooker_publish_post_default']=false;
		...
		$options['wordbooker_share_button_category']=false;
		$options['wordbooker_share_button_frontpage']=false;
		$options['wordbooker_share_button_page']=false;
		$options['wordbooker_share_button_post']=false;
		$options["wordbooker_status_update"]=null;
		$options["wordbooker_status_update_text"]= __(": New blog post :  %title% - %link%", 'wordbooker');
		$options['wordbooker_time_button']=false;
		$options['wordbooker_token_log']=false;
		$options['wordbooker_unlink']=false;
		$options['wordbooker_use_curl']=false;
		$options['wordbooker_use_curl_4']='';
		$options['wordbooker_use_facebook_comments']=false;
		$options['wordbooker_use_fb_comments']=false;
		$options['wordbooker_use_this_image']=false;
		$options['wordbooker_use_url_not_slug']=false;
	}
	return $options;
}
...
function wordbooker_option_manager() {
	global $ol_flash, $wordbooker_settings, $_POST, $user_ID,$wpdb, $blog_id,$wordbooker_user_settings_id,$wordbooker_hook;
	echo '<div class="wrap">';
	...
	$got_id=0;
	 if ( isset($result[0]->user_ID)) {
		$wbuser = wordbooker_get_userdata($result[0]->user_ID);
 		if (strlen($wbuser->access_token)>50) { $got_id=1;}
	}
	if ($got_id==1) {
...
function wordbooker_display_buttons() {
	if (current_user_can('activate_plugins')) {
		echo "<p>";
		echo '<input type="submit" name="SBLO" value="'.__("Save Blog Level Options", 'wordbooker').'" class="button-primary"  />&nbsp;&nbsp;&nbsp;';
		echo '<input type="submit" name="RSD" value="'.__("Reset All Defaults", 'wordbooker').'" class="button-primary" action="poo" />';
		echo "<br /></p>";
	}
}

function wordbooker_blog_level_options() {
	global $ol_flash, $wordbooker_settings, $_POST, $user_ID,$wpdb, $blog_id,$wordbooker_user_settings_id,$wordbooker_hook, $screen_layout_columns;
	...
	add_meta_box('wb_opt5', __('Post Type / Post Category and Tag Options','wordbooker'),   'wordbooker_category_tag_post_options', $wordbooker_hook, 'normal', 'core');
	echo'<p><hr><h3>';
...
<div id="post-body-content" class="has-sidebar-content">
	<?php do_meta_boxes($wordbooker_hook, 'normal', ''); ?>
</div>
</div>
<br class="clear"/>
</div>
<?php
//wordbooker_display_buttons();
		echo '</p></form><br /></div><hr>';
	}

function wordbooker_blog_posting_options() {
		global $ol_flash, $wordbooker_settings, $_POST, $user_ID,$wpdb, $blog_id,$wordbooker_user_settings_id;
		$checked_flag=array('on'=>'checked','off'=>'', 100=>'', 200=>'checked',''=>'');
		...
		if(!isset($wordbooker_settings['wordbooker_publish_no_user'])) {$wordbooker_settings['wordbooker_publish_no_user']="off";}
		...
		if(!isset($wordbooker_settings['wordbooker_allow_publish_select'])) {$wordbooker_settings['wordbooker_allow_publish_select']="off";}
		...
		if(!isset($wordbooker_settings['wordbooker_attribute'])) {$wordbooker_settings['wordbooker_attribute']=" ";}
		...
		if(!isset($wordbooker_settings['wordbooker_status_update_text'])) {$wordbooker_settings['wordbooker_status_update_text']=" ";}
		...
		echo '<br /><label for="wb_action_link">'.__("Action Link Option ", 'wordbooker'). ': </label><select id="wordbooker_actionlink" name="wordbooker_settings[wordbooker_actionlink]"  >';
      	//	if($wordbooker_settings['wordbooker_actionlink']==200) {$wordbooker_settings['wordbooker_actionlink']=100;}
      		 $arr = array(100=> __("None", 'wordbooker'),110=> __("FB Internal (Forces Post as Share)", 'wordbooker'),200=> __("Share Link ", 'wordbooker'),  300=>__("Read Full Article", 'wordbooker'));
                ...
                echo "</select><br />";
        if(!isset($wordbooker_settings['wordbooker_search_this_header'])) {$wordbooker_settings['wordbooker_search_this_header']="off";}
		...
		echo "</select><br />";
		if(!isset($wordbooker_settings['wordbooker_delete_published'])) {$wordbooker_settings['wordbooker_delete_published']="off";}
		echo '<label for="wb_delete_published">'.__("Delete Post from Facebook when Post is deleted from Blog", 'wordbooker'). ' : </label>';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_delete_published]" '.$checked_flag[$wordbooker_settings["wordbooker_delete_published"]].' ><br />';
	wordbooker_display_buttons();
}
...
function wordbooker_blog_facebook_options() {
		...
		$fblike_button=array('button_count'=>'Button Count ','standard'=>'Standard ','box_count'=>'Box Count','button'=>'Button');
		$fbshare_button=array('button_count'=>'Button Count ','icon'=>'Icon ','icon_link'=>'Icon Link','link'=>'Link','box_count'=>'Box Count','button'=>'Button');
		...
		$fblike_send_combi=array('false'=>__('No - use Separate Share Button as configured below','wordbooker'),'true'=>__('Yes - Combine both Like and Share','wordbooker'));
		...
		if(!isset($wordbooker_settings["wordbooker_no_like_stick"])) $wordbooker_settings["wordbooker_no_like_stick"]='off';
		...
		echo '<label for="wb_fblike_send_combi">&nbsp;'.__('Combine Share with Like', 'wordbooker').' :</label> <select id="wordbooker_fblike_send_combi" name="wordbooker_settings[wordbooker_fblike_send_combi]"  >';
		...
		echo '<br /><hr><br /><label for="wb_facebook_share_on">'.__("Include a Facebook Share button in blog", 'wordbooker'). ' : </label>';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_share_on]" '.$checked_flag[$wordbooker_settings["wordbooker_share_on"]].' ><br />';
		...
		echo '<label for="wb_fbshare_align">&nbsp;'.__('Facebook Share - Align Button ', 'wordbooker').' :</label> <select id="wordbooker_fbshare_align" name="wordbooker_settings[wordbooker_fbshare_align]"  >';
		foreach ($fblike_align as $i => $value) {
			if ($i==$wordbooker_settings['wordbooker_fbshare_align']){ print '<option selected="yes" value="'.$i.'" >'.$fblike_align[$i].'</option>';}
		       else {print '<option value="'.$i.'" >'.$fblike_align[$i].'</option>';}}
		echo "</select><br />";

		echo '<label for="wb_fbshare_button">&nbsp;'.__('Facebook Share - Layout Style', 'wordbooker').' :</label> <select id="wordbooker_fbshare_button" name="wordbooker_settings[wordbooker_fbshare_button]"  >';
		foreach ($fbshare_button as $i => $value) {
			if ($i==$wordbooker_settings['wordbooker_fbshare_button']){ print '<option selected="yes" value="'.$i.'" >'.$fbshare_button[$i].'</option>';}
		       else {print '<option value="'.$i.'" >'.$fbshare_button[$i].'</option>';}}
		echo "</select><br />";


		if (!is_numeric($wordbooker_settings['wordbooker_share_width']) || $wordbooker_settings['wordbooker_share_width'] <0) {$wordbooker_settings['wordbooker_share_width']=250;}
		echo '<label for="wb_facebook_share_width">&nbsp;'.__("Width of Facebook Share box", 'wordbooker'). ' : </label>';
		echo '<INPUT TYPE=text NAME="wordbooker_settings[wordbooker_share_width]"  size="7"value="'.$wordbooker_settings["wordbooker_share_width"].'" ><br />';
...
function wordbooker_blog_comment_options() {
		...
		if(!isset($wordbooker_settings['wordbooker_comment_handling'])) {$wordbooker_settings['wordbooker_comment_handling']="off";}
		if(!isset($wordbooker_settings['wordbooker_comment_pull'])) {$wordbooker_settings['wordbooker_comment_pull']="off";}
		if(!isset($wordbooker_settings['wordbooker_comment_push'])) {$wordbooker_settings['wordbooker_comment_push']="off";}
		...
		if(!isset($wordbooker_settings['wordbooker_comment_approve'])) {$wordbooker_settings['wordbooker_comment_approve']="off";}
		...
		if (!isset($wordbooker_settings["wordbooker_comment_attribute"])) {
			$wordbooker_settings["wordbooker_comment_attribute"]=__("[Comment imported from blog] - You Should Edit This",'wordbooker');
		}
		...
		if ($close_comments) {
			$cdostring=sprintf(__("(Currently set to %s days)",'wordbooker'),$close_days_old);
		} else {$cdostring=__("(Currently not set)",'wordbooker'); }
		echo " ".$cdostring.'<br />';
		if(!isset($wordbooker_settings['wordbooker_delete_comment_fb_imp'])) {$wordbooker_settings['wordbooker_delete_comment_fb_imp']="off";}
		if(!isset($wordbooker_settings['wordbooker_delete_comment_fb_exp'])) {$wordbooker_settings['wordbooker_delete_comment_fb_exp']="off";}
		if(!isset($wordbooker_settings['wordbooker_use_fb_comments'])) {$wordbooker_settings['wordbooker_use_fb_comments']="off";}
		echo '<label for="wb_delete_comment_fb_imp">'.__("Delete original comment from Facebook when imported comment is deleted from blog", 'wordbooker'). ' :</label> ';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_delete_comment_fb_imp]" '.$checked_flag[$wordbooker_settings["wordbooker_delete_comment_fb_imp"]].' /><br />';
		echo '<label for="wb_delete_comment_fb_exp">'.__("Delete exported comment from Facebook when original comment is deleted from blog", 'wordbooker'). ' :</label> ';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_delete_comment_fb_exp]" '.$checked_flag[$wordbooker_settings["wordbooker_delete_comment_fb_exp"]].' /><br />';
		...
		echo "</select><br />";
		if(!isset($wordbooker_settings['wordbooker_fb_comments_admin'])) {$wordbooker_settings['wordbooker_fb_comments_admin']="off";}
		echo '<label for="wb_use_fb_comments_admin">'.__("All Wordbooker users can moderate comments  ", 'wordbooker'). ' : </label> ';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_fb_comments_admin]" '.$checked_flag[$wordbooker_settings["wordbooker_fb_comments_admin"]].' /> <br />';
		...
		if(!isset($wordbooker_settings['fb_comment_box_notify'])) {$wordbooker_settings['fb_comment_box_notify']="off";}
		if(!isset($wordbooker_settings['fb_comment_box_import'])) {$wordbooker_settings['fb_comment_box_import']="off";}
		...
		echo '<label for="wordbooker_comment_import">'.__('Automatically import FB Box comments into Wordbooker', 'wordbooker').' : </label>';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[fb_comment_box_import]" '.$checked_flag[$wordbooker_settings["fb_comment_box_import"]].'><br />';
...
function wordbooker_blog_advanced_options() {
		global $ol_flash, $wordbooker_settings, $_POST, $user_ID,$wpdb, $blog_id,$wordbooker_user_settings_id,$table_prefix;
		$checked_flag=array('on'=>'checked','off'=>'', 100=>'', 200=>'checked',''=>'');
        ...
        if(!isset($wordbooker_settings['wordbooker_clear_diagnostics'])) {$wordbooker_settings['wordbooker_clear_diagnostics']="off";}
        if(!isset($wordbooker_settings['wordbooker_token_log'])) {$wordbooker_settings['wordbooker_token_log']="off";}
        if(!isset($wordbooker_settings['wordbooker_comment_log'])) {$wordbooker_settings['wordbooker_comment_log']="off";}
        if(!isset($wordbooker_settings['wordbooker_override_author'])) {$wordbooker_settings['wordbooker_override_author']="off";}
		if(!isset($wordbooker_settings['wordbooker_disable_shorties'])) {$wordbooker_settings['wordbooker_disable_shorties']="off";}
        if(!isset($wordbooker_settings['wordbooker_fb_rec_act'])) {$wordbooker_settings['wordbooker_fb_rec_act']="off";}
        if(!isset($wordbooker_settings['wordbooker_iframe'])) {$wordbooker_settings['wordbooker_iframe']="off";}
        if(!isset($wordbooker_settings['wordbooker_use_url_not_slug'])) {$wordbooker_settings['wordbooker_use_url_not_slug']="off";}
		if(!isset($wordbooker_settings['wordbooker_meta_tag_scan'])) {$wordbooker_settings['wordbooker_meta_tag_scan']="off";}
        if(!isset($wordbooker_settings['wordbooker_meta_tag_scan'])) {$wordbooker_settings['wordbooker_meta_tag_scan']="off";}
		if(!isset($wordbooker_settings['wordbooker_meta_tag_thumb'])) {$wordbooker_settings['wordbooker_meta_tag_thumb']="off";}
        if(!isset($wordbooker_settings['wordbooker_use_this_image'])) {$wordbooker_settings['wordbooker_use_this_image']="off";}
        if(!isset($wordbooker_settings['wordbooker_fb_disable_og'])) {$wordbooker_settings['wordbooker_fb_disable_og']="off";}
        if(!isset($wordbooker_settings['wordbooker_fb_disable_api'])) {$wordbooker_settings['wordbooker_fb_disable_api']="off";}
        if(!isset($wordbooker_settings['wordbooker_fake_publish'])) {$wordbooker_settings['wordbooker_fake_publish']="off";}
        if(!isset($wordbooker_settings['wordbooker_disabled'])) {$wordbooker_settings['wordbooker_disabled']="off";}
        if(!isset($wordbooker_settings['wordbooker_use_curl'])) {$wordbooker_settings['wordbooker_use_curl']="off";}
        if(!isset($wordbooker_settings['wordbooker_can_curl'])) {$wordbooker_settings['wordbooker_can_curl']=0;}
        if(!isset($wordbooker_settings['wordbooker_use_curl_4'])) {$wordbooker_settings['wordbooker_use_curl_4']="off";}
        if(!isset($wordbooker_settings['wordbooker_images_double_quote'])) {$wordbooker_settings['wordbooker_images_double_quote']="off";}
        ...
        $fbshortie_control=array('off'=>__('Use permalinks as set in the Permalinks Settings page for the blog','wordbooker'),'short'=>__('Use Short Code permalinks as provided by JetPack or other plugin','wordbooker'),'forced'=>__('Always use /?p=1234 style links','wordbooker'));
		echo '<label for="wb_wordbooker_disable_shorties">'.__("Permalink structure for URLs posted to Facebook", 'wordbooker'). ' : </label> <select id="wb_wordbooker_disable_shorties" name="wordbooker_settings[wordbooker_disable_shorties]"  >';

		foreach ($fbshortie_control as $i => $value) {
			if ($i==$wordbooker_settings['wordbooker_disable_shorties']){ print '<option selected="yes" value="'.$i.'" >'.$fbshortie_control[$i].'</option>';}
		       else {print '<option value="'.$i.'" >'.$fbshortie_control[$i].'</option>';}}
		echo "</select></P><p>";
		...
		echo '<label for="wb_wordbooker_disable_ogtags">'.__("Only find images enclosed in Double Quotes", 'wordbooker'). ' : </label>';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_images_double_quote]" '.$checked_flag[$wordbooker_settings["wordbooker_images_double_quote"]].' ></P><p>';
		...
		if ($wordbooker_settings['wordbooker_can_curl']>0){
		  echo '<label for="wordbooker_use_curl">'.__("Use Curl for Facebook interface calls", 'wordbooker'). ' : </label>';
		  echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_use_curl]" '.$checked_flag[$wordbooker_settings["wordbooker_use_curl"]].' ></P><p>';
		  if (WORDBOOKER_IPV=='6'){
	...
	//	 if ($blog_id>1){
	//	  echo '<label for="wordbooker_unlink">'.__("Disable Cross Site User Association for this blog", 'wordbooker'). ' : </label>';
	//	  echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wordbooker_unlink]" '.$checked_flag[$wordbooker_settings["wordbooker_unlink"]].' ></P><p>';
	//	  }
...
function wordbooker_category_tag_post_options() {
		global $ol_flash, $wordbooker_settings, $_POST, $user_ID,$wpdb, $blog_id,$wordbooker_user_settings_id,$table_prefix;
		$checked_flag=array('on'=>'checked','off'=>'', 100=>'', 200=>'checked',''=>'');
		echo "<br /><p>";
		$ptypes=get_post_types(array('_builtin' => false));
		 $poststringon="";
		  $poststringoff="";
?><script type="text/javascript"> function moveOption( fromID, toID, idx,inclist,exlist )
{
    ...
    document.getElementById( inclist ).value="";
    document.getElementById( exlist ).value="";
   for (var x = document.getElementById( fromID ).options.length - 1; x >= 0 ; x--) {
    document.getElementById( inclist ).value=document.getElementById( inclist ).value+","+document.getElementById( fromID ).options[x].value;
    }
       for (var x = document.getElementById( toID ).options.length - 1; x >= 0 ; x--) {
   document.getElementById( exlist).value=document.getElementById( exlist ).value+","+document.getElementById( toID ).options[x].value;
    }
}
function moveOptions( fromID, toID,inclist,exlist )
{
    for (var x = document.getElementById( fromID ).options.length - 1; x >= 0 ; x--)
    {
        if (document.getElementById( fromID ).options[x].selected == true)
        {
            moveOption( fromID, toID, x, inclist,exlist);
        }
    }
}
</script>
...
$workliston="";
$worklistoff="";
$pincludes=explode(",",$wordbooker_settings['wordbooker_include_ptype']);
$cexcludes=explode(",",$wordbooker_settings['wordbooker_exclude_ctype']);
$texcludes=explode(",",$wordbooker_settings['wordbooker_exclude_ttype']);
$catlimit=1;
if (isset($wordbooker_settings["wb_show_subcat"])) {$catlimit=999999999999999999;}
      foreach ($ptypes as $ptype) {
		if (in_array($ptype,$pincludes)){
			$poststringon.='<option value="'.$ptype.'">'.$ptype.'</option>';
			$workliston.=",".$ptype;
	     }
			else { $poststringoff.='<option value="'.$ptype.'">'.$ptype.'</option>';
			$worklistoff.=",".$ptype;
	    }
	   }
		wordbooker_set_option('wordbooker_include_ptype',$workliston);
		wordbooker_set_option('wordbooker_exclude_ptype',$worklistoff);
		$workliston="";
		$worklistoff="";
		$ctypes=get_categories(array(  'orderby' => 'name',  'order' => 'ASC',  'hide_empty'=>0, ));
		foreach ($ctypes as $ctype) {
			if ($ctype->parent < $catlimit) {
				if (in_array($ctype->term_id ,$cexcludes)){
					$catstringoff.='<option value="'.$ctype->term_id .'">'.rtrim(get_category_parents($ctype->term_id, FALSE, ' &raquo; ',TRUE),' &raquo; ').'</option>';
					$worklistoff.=",".$ctype->term_id ;
				}
				else { $catstringon.='<option value="'.$ctype->term_id .'">'.rtrim(get_category_parents($ctype->term_id, FALSE, ' &raquo; ',TRUE),' &raquo; ').'</option>';
					$workliston.=",".$ctype->term_id ;
				}
			}
		}
		wordbooker_set_option('wordbooker_include_ctype',$workliston);
		wordbooker_set_option('wordbooker_exclude_ctype',$worklistoff);
		$workliston="";
		$worklistoff="";
		$ttypes=get_tags();
		foreach ($ttypes as $ttype) {
			if (in_array($ttype->term_id,$texcludes)){
				$tagstringoff.='<option value="'.$ttype->term_id .'">'.$ttype->name.'</option>';
				$worklistoff.=",".$ttype->term_id ;
			}
			else { $tagstringon.='<option value="'.$ttype->term_id .'">'.$ttype->name.'</option>';
				$workliston.=",".$ttype->term_id ;
			}
		}
		wordbooker_set_option('wordbooker_include_ttype',$workliston);
		wordbooker_set_option('wordbooker_exclude_ttype',$worklistoff);
		$workliston="";
		$worklistoff="";
	    echo "<p><table><tr><th>".__("Excluded Post Types", 'wordbooker')."</th><th></th><th>".__("Included Post Types", 'wordbooker')."</th><th></th></tr><tr><td>";
	    echo '<select id="wordbooker_exclude_ptype" size="12" multiple="multiple" style="min-width:20em; width:20em">';
	    echo $poststringoff.'</select></td><td align="center" valign="center">';
	    echo '<input type="button" value="--&gt;" onclick="moveOptions(\'wordbooker_exclude_ptype\', \'wordbooker_include_ptype\',\'wep\',\'wip\');"><br>';
	    echo '<input type="button" value="&lt;--" onclick="moveOptions(\'wordbooker_include_ptype\',\'wordbooker_exclude_ptype\',\'wip\',\'wep\');"></td><td>';
	    echo '<select id="wordbooker_include_ptype" size="12" multiple="multiple" style="min-width:20em; width:20em">';
	    echo $poststringon.'</select></td></tr></table>';
		echo '<input type="hidden" id="wep" name="wordbooker_settings[wordbooker_exclude_ptype]"  value="'.$wordbooker_settings["wordbooker_exclude_ptype"].'"/>';
		echo '<input type="hidden" id="wip" name="wordbooker_settings[wordbooker_include_ptype]"  value="'.$wordbooker_settings["wordbooker_include_ptype"].'" />';

	    echo "</p><p><br /><table><tr><th>".__("Excluded Category Types", 'wordbooker')."</th><th></th><th>".__("Included Category Types", 'wordbooker')."</th><th></th></tr><tr><td>";
	    echo '<select id="wordbooker_exclude_ctype" size="12" multiple="multiple" style="min-width:20em; width:20em">';
	    echo $catstringoff.'</select></td><td align="center" valign="center">';
	    echo '<input type="button" value="--&gt;" onclick="moveOptions(\'wordbooker_exclude_ctype\', \'wordbooker_include_ctype\',\'cep\',\'cip\');"><br>';
	    echo '<input type="button" value="&lt;--" onclick="moveOptions(\'wordbooker_include_ctype\',\'wordbooker_exclude_ctype\',\'cip\',\'cep\');"></td><td>';
	    echo '<select id="wordbooker_include_ctype" size="12" multiple="multiple" style="min-width:20em; width:20em">';
	    echo $catstringon.'</select></td></tr></table>';
		echo '<input type="hidden" id="cep" name="wordbooker_settings[wordbooker_exclude_ctype]"  value="'.$wordbooker_settings["wordbooker_exclude_ctype"].'"/>';
		echo '<input type="hidden" id="cip" name="wordbooker_settings[wordbooker_include_ctype]"  value="'.$wordbooker_settings["wordbooker_include_ctype"].'" />';
		echo '<label for="wb_show_subcat">'.__("Show sub categories", 'wordbooker'). ' : </label>';
		if(!isset($wordbooker_settings['wb_show_subcat'])) {$wordbooker_settings['wb_show_subcat']="off";}
		if(!isset($wordbooker_settings['wb_proc_exclude'])) {$wordbooker_settings['wb_proc_exclude']="off";}
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wb_show_subcat]" '.$checked_flag[$wordbooker_settings["wb_show_subcat"]].' /><br />';
		echo '<label for="wb_proc_exclude">'.__("Only process Excludes when posting", 'wordbooker'). ' :  </label>';
		echo '<INPUT TYPE=CHECKBOX NAME="wordbooker_settings[wb_proc_exclude]" '.$checked_flag[$wordbooker_settings["wb_proc_exclude"]].' />';
	   echo "</p><p><br /><table><tr><th>".__("Excluded Tag Types", 'wordbooker')."</th><th></th><th>".__("Included Tag Types", 'wordbooker')."</th><th></th></tr><tr><td>";
	    echo '<select id="wordbooker_exclude_ttype" size="12" multiple="multiple" style="min-width:20em; width:20em">';
	    echo $tagstringoff.'</select></td><td align="center" valign="center">';
	    echo '<input type="button" value="--&gt;" onclick="moveOptions(\'wordbooker_exclude_ttype\', \'wordbooker_include_ttype\',\'tep\',\'tip\');"><br>';
	    echo '<input type="button" value="&lt;--" onclick="moveOptions(\'wordbooker_include_ttype\',\'wordbooker_exclude_ttype\',\'tip\',\'tep\');"></td><td>';
	    echo '<select id="wordbooker_include_ttype" size="12" multiple="multiple" style="min-width:20em; width:20em">';
	    echo $tagstringon.'</select></td></tr></table></p>';
		echo '<input type="hidden" id="tep" name="wordbooker_settings[wordbooker_exclude_ttype]" value="'.$wordbooker_settings["wordbooker_exclude_ttype"].'"/>';
		echo '<input type="hidden" id="tip" name="wordbooker_settings[wordbooker_include_ttype]" value="'.$wordbooker_settings["wordbooker_include_ttype"].'" />';
		wordbooker_display_buttons();
}

function wordbooker_user_level_options(){
		global $ol_flash, $wordbooker_settings, $_POST, $user_ID,$wpdb, $blog_id,$wordbooker_user_settings_id,$user_ID,$wordbooker_hook;
		# USER LEVEL OPTIONS
		$checked_flag=array('on'=>'checked','off'=>'', 100=>'', 200=>'checked',''=>'');
		...
		if(!isset($wordbookeruser_settings['wordbooker_publish_default'])) {$wordbookeruser_settings['wordbooker_publish_default']="off";}
		if(!isset($wordbookeruser_settings['wordbooker_publish_post_default'])) {$wordbookeruser_settings['wordbooker_publish_post_default']="off";}
		if(!isset($wordbookeruser_settings['wordbooker_publish_page_default'])) {$wordbookeruser_settings['wordbooker_publish_page_default']="off";}
        if(!isset($wordbookeruser_settings['wordbooker_excerpt_for_attribute'])) {$wordbookeruser_settings['wordbooker_excerpt_for_attribute']="off";}
	...
	if(!isset($wordbookeruser_settings['wordbooker_primary_active'])) {$wordbookeruser_settings['wordbooker_primary_active']="off";}
	...
	echo '&nbsp;<INPUT TYPE=CHECKBOX NAME="wordbooker_primary_active" '.$checked_flag[$wordbookeruser_settings["wordbooker_primary_active"]].'></p><p>';
		...
		if (is_array($fanpages2)){
			$have_fan_pages=1;
			if(!isset($wordbookeruser_settings['wordbooker_secondary_target'])) {$wordbookeruser_settings['wordbooker_secondary_target']="";}
	...
	echo "</select>";
		if(!isset($wordbookeruser_settings['wordbooker_secondary_active'])) {$wordbookeruser_settings['wordbooker_secondary_active']="off";}
		...
		echo '<label for="wb_action_link">'.__('Action Link Option', 'wordbooker').' : </label><select id="wordbooker_actionlink" name="wordbooker_actionlink"  >';
 		if(!isset($wordbookeruser_settings['wordbooker_actionlink'])) {$wordbookeruser_settings['wordbooker_actionlink']="200";}
        		 $arr = array(100=> __("None", 'wordbooker'),110=> __("FB Internal (Forces Post as Share)", 'wordbooker'),200=> __("Share Link ", 'wordbooker'),  300=>__("Read Full Article", 'wordbooker'));
                 foreach ($arr as $i => $value) {
		...
		if(!isset($wordbookeruser_settings['wordbooker_search_this_header'])) {$wordbookeruser_settings['wordbooker_search_this_header']="0";}
       		$arr = array(0=> __("Same as Blog", 'wordbooker'), 100=> __("No", 'wordbooker'),  200=> __("Yes", 'wordbooker'));
                foreach ($arr as $i => $value) {
		...
		if ( function_exists( 'get_the_post_thumbnail' ) ) {
			...
			if(!isset($wordbookeruser_settings['wordbooker_thumb_only'])) {$wordbookeruser_settings['wordbooker_thumb_only']="off";}
		...
		if(!isset($wordbookeruser_settings['wordbooker_use_excerpt'])) {$wordbookeruser_settings['wordbooker_use_excerpt']="off";}
		...
		$option="";
		if ($have_fan_pages==1) {
			if(!isset($wordbookeruser_settings['wordbooker_status_id'])) {$wordbookeruser_settings['wordbooker_status_id']="0";}
			foreach ($fanpages as $fan_page) {
		...
		echo '</select><br />';
		if(!isset($wordbookeruser_settings['wordbooker_disable_status'])) {$wordbookeruser_settings['wordbooker_disable_status']="off";}
		...
		echo '<label for="wb_friend_list">'.__('Limit Friend tagging to ', 'wordbooker').' : </label> <select name="wordbooker_friend_list" ><option selected="yes" value=-200>'.__('No Friends (Disabled)', 'wordbooker').'&nbsp;&nbsp;</option>';
		$option="";
		if(!isset($wordbookeruser_settings['wordbooker_friend_list'])) {$wordbookeruser_settings['wordbooker_friend_list']="0";}
		if ($wordbookeruser_settings["wordbooker_friend_list"]==-100 ) {
		$option .= '<option selected="yes" value=-100>'.__('All friends', 'wordbooker').'&nbsp;&nbsp;</option>';} else { $option .= '<option value=-100>'.__('All friends', 'wordbooker').'&nbsp;&nbsp;</option>';}
		$sql=$wpdb->prepare("SELECT flid,name FROM ".WORDBOOKER_FB_FRIEND_LISTS." where user_id=%d and list_type='friend' order by name ASC ",$user_ID);
		$r = $wpdb->get_results($sql,ARRAY_A);

		foreach($r as $rw)
		{
		      if ($rw['flid']==$wordbookeruser_settings["wordbooker_friend_list"] ) {$option .= '<option selected="yes" value='.$rw['flid'].'>';} else { $option .= '<option value='.$rw['flid'].'>';}
		      $option .= $rw['name']."&nbsp;&nbsp;";
		      $option .= '</option>';}
			echo $option;
		echo '</select><br /><p>';
		...
		if (!defined('WORDBOOKER_PREMIUM')) {} else {
			if(!isset($wordbookeruser_settings['wordbooker_album_upload'])) {$wordbookeruser_settings['wordbooker_album_upload']="off";}
			echo '<label for="wb_album_list"> <INPUT TYPE=CHECKBOX NAME="wordbooker_album_upload" '.$checked_flag[$wordbookeruser_settings["wordbooker_album_upload"]].'>&nbsp;&nbsp;'.__('Upload photos to  ', 'wordbooker').' : </label> <select name="wordbooker_album_list" ><option selected="yes" value=-200>'.__('No Album (Disabled)', 'wordbooker').'&nbsp;&nbsp;</option>';
		$option="";
		if(!isset($wordbookeruser_settings['wordbooker_album_list'])) {$wordbookeruser_settings['wordbooker_album_list']="0";}
		...
		foreach($r as $rw)
		{
		      if ($rw['flid']==$wordbookeruser_settings["wordbooker_album_list"] ) {$option .= '<option selected="yes" value='.$rw['flid'].'>';} else { $option .= '<option value='.$rw['flid'].'>';}
		      $option .= $rw['name']."&nbsp;&nbsp;";
		      $option .= '</option>';}
			echo $option;
		echo '</select><br /><p>';
